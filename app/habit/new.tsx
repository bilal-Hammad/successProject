...
export default function NewHabitScreen() {
  ...
  const handleSave = async () => {
    ...
    if (days.length === 0) { Alert.alert(t('form.selectDays'), t('form.selectDaysMsg')); return; }
    ...
    if (habit.reminderTime) await scheduleHabitReminder(habit);
    router.back();
  };
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={[styles.scroll, { padding: theme.spacing.md }]}>
          ...
          <TextInput value={title} onChangeText={setTitle} placeholder={t('form.habitNamePlaceholder')} placeholderTextColor={theme.colors.textSecondary} style={inputStyle} />
          ...
          <View style={styles.colorRow}>
            {COLORS.map((c) => (
              <Pressable key={c} onPress={() => setColor(c)} style={[styles.colorDot, { backgroundColor: c, borderWidth: color === c ? 3 : 0, borderColor: '#fff', shadowColor: color === c ? c : 'transparent', shadowOpacity: 0.6, shadowRadius: 4, elevation: color === c ? 4 : 0 }]} />
            ))}
          </View>
          ...
          <Text style={[styles.label, { color: theme.colors.textSecondary, marginTop: 16 }]}>{t('form.reminderOptional')}</Text>
          ...
          <Pressable onPress={handleSave} style={[styles.saveBtn, { backgroundColor: theme.colors.primary, borderRadius: theme.radius.md }]}>
            <Text style={styles.saveBtnText}>{t('form.saveHabit')}</Text>
          </Pressable>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
